...
const typeConfig: Record<Notification["type"], { icon: typeof Bell; color: string; label: string }> = {
  friend_request: { icon: UserPlus,      color: "bg-primary-500", label: "sent you a friend request" },
  post_like:      { icon: Heart,         color: "bg-red-500",     label: "liked your post" },
  post_comment:   { icon: MessageCircle, color: "bg-green-500",   label: "commented on your post" },
  message:        { icon: MessageCircle, color: "bg-blue-500",    label: "sent you a message" },
};
...
export default function NotificationsPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {loading && <div className="flex justify-center py-12"><Loader2 className="w-6 h-6 animate-spin text-primary-600" /></div>}
      ...
      {!loading && notifs.length > 0 && (
        <div className="card overflow-hidden divide-y divide-gray-100 dark:divide-gray-800">
          {notifs.map((notif) => {
            ...
            return (
              <Link key={notif.id} href={href} onClick={() => !notif.read && markRead(notif.id)}
                className={cn("flex items-start gap-3 px-4 py-3 transition-colors hover:bg-gray-50 dark:hover:bg-gray-800/50",
                  !notif.read && "bg-primary-50/50 dark:bg-primary-900/10")}>
                <div className="relative flex-shrink-0">
                  <div className="w-10 h-10 bg-gray-200 dark:bg-gray-700 rounded-full flex items-center justify-center">
                    <span className="text-sm font-bold text-gray-600 dark:text-gray-300">{notif.actorName?.charAt(0)?.toUpperCase() ?? "?"}</span>
                  </div>
                  <span className={cn("absolute -bottom-0.5 -right-0.5 w-5 h-5 rounded-full flex items-center justify-center", cfg.color)}>
                ...
                {!notif.read && <div className="w-2.5 h-2.5 bg-primary-600 rounded-full flex-shrink-0 mt-1.5" />}
              </Link>
            );
          })}
        </div>
