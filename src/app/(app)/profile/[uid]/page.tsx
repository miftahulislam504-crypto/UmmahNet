...
export default function ProfilePage() {
  ...
  // Image posts only (for Photos tab)
    ...
    <>
      <div className="flex flex-col">
        ...
        <div className="card rounded-t-none border-t-0 min-h-40 p-4">
          ...
          {tab === "photos" && (
            <>
              ...
              {!postsLoading && photoPosts.length === 0 && (
                <EmptyState icon={Grid3X3} message="No photos yet." />
              )}
              {!postsLoading && photoPosts.length > 0 && (
              ...
              {!postsLoading && videoPosts.length > 0 && (
                <div className="grid grid-cols-2 gap-2">
                  {videoPosts.map((p) => (
                    ...
                    >
                      {p.mediaUrls?.[0] && (
                        <Image src={p.mediaUrls[0]} alt="" fill className="object-cover" />
                      )}
          ...
          {/* Liked tab */}
          {tab === "liked" && (
            <EmptyState icon={Heart} message="Liked posts are private." />
          )}
        </div>
      </div>
